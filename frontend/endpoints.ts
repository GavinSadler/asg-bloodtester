...
export function getRecentTemperatureData() {
  return fetch(
    `${toplevel}/discoveryqproxy/php/mysql2json.php?database=View&table=temperature_recent`,
  )
    .then((res) => res.json())
    .then((res) => {
      return res as [
        { timestamp: string; device: string; temperature: string },
      ];
    });
}
